...
")
  ...
  # This MUST be set before Microsoft.Common.props is imported (MSB3539)
...
")
  file(GENERATE
    OUTPUT "${_PROJECT_DIR}/Directory.Build.props"
...
function(sps_add_dotnet_test)
  ...
  # Build Compile Include items from source files (use absolute paths)
  set(_COMPILE_ITEMS "")
  foreach(_SOURCE ${TEST_SOURCES})
    get_filename_component(_SOURCE_ABS "${_SOURCE}" ABSOLUTE
      BASE_DIR "${CMAKE_CURRENT_SOURCE_DIR}")
    string(APPEND _COMPILE_ITEMS "    <Compile Include=\"${_SOURCE_ABS}\" />\n")
  endforeach()

  # Scripts directory (config-independent)
  set(_SCRIPTS_DIR "${CMAKE_BINARY_DIR}/dotnet-scripts")
  file(MAKE_DIRECTORY "${_SCRIPTS_DIR}")

  # Generate CMake script to create project files at build time
  set(_CREATE_SCRIPT "${_SCRIPTS_DIR}/${TEST_NAME}_create.cmake")
  file(WRITE "${_CREATE_SCRIPT}"
"# Create test project files for ${TEST_NAME}
# Generated by spsDotNet.cmake - do not edit
cmake_minimum_required(VERSION 3.16)
...
# Directory.Build.props - must set BaseIntermediateOutputPath early (MSB3539)
file(WRITE \"\${PROJECT_DIR}/Directory.Build.props\"
\"<Project>
  <PropertyGroup>
...
# .csproj file with source file references
file(WRITE \"\${PROJECT_DIR}/\${PROJECT_NAME}.csproj\"
\"<Project Sdk=\\\"Microsoft.NET.Sdk\\\">

  <PropertyGroup>
    <OutputType>Exe</OutputType>
    <TargetFramework>${TEST_TFM}</TargetFramework>
    <RuntimeIdentifier>${SPS_DOTNET_RID}</RuntimeIdentifier>
    <ImplicitUsings>disable</ImplicitUsings>
    <Nullable>disable</Nullable>
  ...
  <ItemGroup>
${_COMPILE_ITEMS}  </ItemGroup>

</Project>
\")

message(STATUS \"Created project files for \${PROJECT_NAME}\")
")

  # Target to create project and add package reference at build time
  add_custom_target(${TEST_NAME}_create
    COMMAND ${CMAKE_COMMAND}
      -DPROJECT_DIR=${_TEST_DIR}
      -DPROJECT_NAME=${TEST_NAME}
      -P "${_CREATE_SCRIPT}"
    COMMAND ${SPS_DOTNET_EXECUTABLE} add "${_TEST_DIR}/${TEST_NAME}.csproj" package
      ${TEST_PACKAGE_REF}
...
function(sps_add_dotnet_executable)
  ...
  # Build Compile Include items from source files (use absolute paths)
  set(_COMPILE_ITEMS "")
  foreach(_SOURCE ${EXE_SOURCES})
    get_filename_component(_SOURCE_ABS "${_SOURCE}" ABSOLUTE
      BASE_DIR "${CMAKE_CURRENT_SOURCE_DIR}")
    string(APPEND _COMPILE_ITEMS "    <Compile Include=\"${_SOURCE_ABS}\" />\n")
  endforeach()

  # Set unsafe property if requested
  if(EXE_ALLOW_UNSAFE)
    set(_UNSAFE_PROP "<AllowUnsafeBlocks>true</AllowUnsafeBlocks>")
  else()
    set(_UNSAFE_PROP "")
  endif()
  ...
  # Generate CMake script to create project files at build time
  set(_CREATE_SCRIPT "${_SCRIPTS_DIR}/${EXE_NAME}_create.cmake")
  file(WRITE "${_CREATE_SCRIPT}"
"# Create executable project files for ${EXE_NAME}
# Generated by spsDotNet.cmake - do not edit
cmake_minimum_required(VERSION 3.16)
...
\"<Project Sdk=\\\"Microsoft.NET.Sdk\\\">
  ...
  <PropertyGroup>
    <OutputType>Exe</OutputType>
    <TargetFramework>${EXE_TFM}</TargetFramework>
    <RuntimeIdentifier>${SPS_DOTNET_RID}</RuntimeIdentifier>
    <ImplicitUsings>disable</ImplicitUsings>
    <Nullable>disable</Nullable>
    ${_UNSAFE_PROP}
...
")
  ...
  add_custom_target(${EXE_NAME}_create
    COMMAND ${CMAKE_COMMAND}
      -DPROJECT_DIR=${_EXE_DIR}
      -DPROJECT_NAME=${EXE_NAME}
      -P "${_CREATE_SCRIPT}"
    COMMAND ${SPS_DOTNET_EXECUTABLE} add "${_EXE_DIR}/${EXE_NAME}.csproj" package
      ${EXE_PACKAGE_REF}
      --source "${_PACKAGES_DIR}"
    DEPENDS ${EXE_DEPENDS}
  ...
  add_custom_target(${EXE_NAME}_build ALL
    COMMAND ${SPS_DOTNET_EXECUTABLE} build "${_EXE_DIR}" -c $<CONFIG>
...
message(STATUS "Generated: ${SLN_PATH}")
  ...
  # Generate script to clear only our package from NuGet cache (cross-platform)
  set(_CLEAR_CACHE_SCRIPT "${_SCRIPTS_DIR}/NuGetClearCache.cmake")
  file(WRITE "${_CLEAR_CACHE_SCRIPT}" [=[
# Clear specific package from NuGet cache (cross-platform)
# Generated by spsDotNet.cmake - do not edit
cmake_minimum_required(VERSION 3.16)

# Inputs: DOTNET_EXE, PACKAGE_NAME

# Query NuGet cache location
execute_process(
  COMMAND ${DOTNET_EXE} nuget locals global-packages --list
  OUTPUT_VARIABLE _NUGET_OUTPUT
  OUTPUT_STRIP_TRAILING_WHITESPACE
  RESULT_VARIABLE _NUGET_RESULT
)

if(NOT _NUGET_RESULT EQUAL 0)
  message(FATAL_ERROR "Failed to query NuGet cache location")
endif()

# Parse output: "global-packages: /path/to/cache/"
string(REGEX REPLACE "^global-packages:[ \t]*" "" _CACHE_PATH "${_NUGET_OUTPUT}")
string(STRIP "${_CACHE_PATH}" _CACHE_PATH)
# Remove trailing slashes for clean path construction
string(REGEX REPLACE "[\\\\/]+$" "" _CACHE_PATH "${_CACHE_PATH}")

# NuGet package names are lowercase in the cache
string(TOLOWER "${PACKAGE_NAME}" _PACKAGE_LOWER)
set(_PACKAGE_PATH "${_CACHE_PATH}/${_PACKAGE_LOWER}")

if(EXISTS "${_PACKAGE_PATH}")
  message(STATUS "Removing: ${_PACKAGE_PATH}")
  file(REMOVE_RECURSE "${_PACKAGE_PATH}")
  message(STATUS "Cleared ${PACKAGE_NAME} from NuGet cache")
else()
  message(STATUS "${PACKAGE_NAME} not found in NuGet cache (already clean)")
endif()
]=])

  # Get the main package name from the first registered project
  list(GET _PROJECTS 0 _FIRST_PROJECT)
  string(REPLACE "|" ";" _FIRST_PROJECT_PARTS "${_FIRST_PROJECT}")
  list(GET _FIRST_PROJECT_PARTS 0 _PACKAGE_NAME)

  # Utility target to clear only our package from NuGet cache
  add_custom_target(NuGetClearCache
    COMMAND ${CMAKE_COMMAND}
      -DDOTNET_EXE=${SPS_DOTNET_EXECUTABLE}
      -DPACKAGE_NAME=${_PACKAGE_NAME}
      -P "${_CLEAR_CACHE_SCRIPT}"
    COMMENT "Clearing ${_PACKAGE_NAME} from NuGet cache"
    VERBATIM
  )
