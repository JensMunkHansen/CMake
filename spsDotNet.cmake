...
")
  ...
  # Script to patch .csproj to reference original source file
  set(_SCRIPTS_DIR "${CMAKE_BINARY_DIR}/dotnet-scripts")
  file(MAKE_DIRECTORY "${_SCRIPTS_DIR}")
  set(_PATCH_SCRIPT "${_SCRIPTS_DIR}/${TEST_NAME}_patch.cmake")
  file(WRITE "${_PATCH_SCRIPT}"
"# Patch .csproj to reference original source file
cmake_minimum_required(VERSION 3.16)
file(READ \"\${CSPROJ}\" content)
string(REPLACE \"</PropertyGroup>\" \"<EnableDefaultCompileItems>false</EnableDefaultCompileItems>\\n  </PropertyGroup>\" content \"\${content}\")
string(REPLACE \"</Project>\" \"\\n  <ItemGroup>\\n    <Compile Include=\\\"\${SOURCE}\\\" />\\n  </ItemGroup>\\n</Project>\" content \"\${content}\")
file(WRITE \"\${CSPROJ}\" \"\${content}\")
")
  ...
  add_custom_target(${TEST_NAME}_create
    ...
    COMMAND ${SPS_DOTNET_EXECUTABLE} new console
      ...
      --output "${_TEST_DIR}"
    COMMAND ${CMAKE_COMMAND}
      -DCSPROJ=${_TEST_DIR}/${TEST_NAME}.csproj
      -DSOURCE=${_MAIN_SOURCE_ABS}
      -P "${_PATCH_SCRIPT}"
...
")
  ...
  set(_SCRIPTS_DIR "${CMAKE_BINARY_DIR}/dotnet-scripts")
  file(MAKE_DIRECTORY "${_SCRIPTS_DIR}")
  set(_PATCH_SCRIPT "${_SCRIPTS_DIR}/${EXE_NAME}_patch.cmake")
  file(WRITE "${_PATCH_SCRIPT}"
"# Patch .csproj to reference original source file
cmake_minimum_required(VERSION 3.16)
file(READ \"\${CSPROJ}\" content)
...
")
  ...
  add_custom_target(${EXE_NAME}_create
    ...
    COMMAND ${SPS_DOTNET_EXECUTABLE} new console
      ...
      --output "${_EXE_DIR}"
    COMMAND ${CMAKE_COMMAND}
      -DCSPROJ=${_EXE_DIR}/${EXE_NAME}.csproj
      -DSOURCE=${_MAIN_SOURCE_ABS}
      -P "${_PATCH_SCRIPT}"
