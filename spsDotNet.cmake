...
**********
  ...
  Standard NuGet Package Location:
    On Linux, NuGet packages are output to ``${CMAKE_BINARY_DIR}/packages``.
    This can be overridden by setting ``SPS_DOTNET_PACKAGES_DIR`` before including
    this module.
...
# === Configuration ===
# Use CACHE variable to ensure visibility across add_subdirectory() calls
if(NOT DEFINED SPS_DOTNET_PACKAGES_DIR)
  set(SPS_DOTNET_PACKAGES_DIR "${CMAKE_BINARY_DIR}/packages"
    CACHE PATH "Directory for NuGet package output")
endif()
...
function(sps_add_dotnet_library)
  ...
  set(_PROJECT_DIR "${CMAKE_BINARY_DIR}/dotnet/${DOTNET_NAME}")
  set(_NATIVE_DIR "${_PROJECT_DIR}/native")

  # Create output directories
  file(MAKE_DIRECTORY "${_PROJECT_DIR}")
  file(MAKE_DIRECTORY "${SPS_DOTNET_PACKAGES_DIR}")

  # Generate CMake script to collect runtime dependencies at build time
  set(_COLLECT_SCRIPT "${_PROJECT_DIR}/collect_deps.cmake")
...
"<Project Sdk=\"Microsoft.NET.Sdk\">
  ...
  <PropertyGroup>
    ...
    <PackageOutputPath>${SPS_DOTNET_PACKAGES_DIR}</PackageOutputPath>
  </PropertyGroup>

  <ItemGroup>
    <!-- Native libraries (main target and all dependencies) -->
    <Content Include=\"${_NATIVE_DIR}/$<CONFIG>/*\">
...
")
  ...
  file(GENERATE
    OUTPUT "${_PROJECT_DIR}/$<CONFIG>/${DOTNET_NAME}.csproj"
  ...
  foreach(_SOURCE ${DOTNET_SOURCES})
    ...
    file(GENERATE
      OUTPUT "${_PROJECT_DIR}/$<CONFIG>/${_SOURCE_NAME}"
  ...
  add_custom_target(${DOTNET_NAME}_collect_deps
    COMMAND ${CMAKE_COMMAND}
      ...
      -DOUTPUT_DIR=${_NATIVE_DIR}/$<CONFIG>
  ...
  message(STATUS "  NuGet output: ${SPS_DOTNET_PACKAGES_DIR}")
endfunction()
...
function(sps_add_dotnet_test)
  ...
  set(_TEST_DIR "${CMAKE_BINARY_DIR}/dotnet/tests/${TEST_NAME}")
  ...
  add_custom_target(${TEST_NAME}_create
    ...
    COMMAND ${SPS_DOTNET_EXECUTABLE} add "${_TEST_DIR}" package
      ${TEST_PACKAGE_REF}
      --source "${SPS_DOTNET_PACKAGES_DIR}"
